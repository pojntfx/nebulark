...
build-examples-cpp-json_calculator: build-wasi-sdk-container
	@docker run --rm -v ${PWD}:/root/go/src/github.com/pojntfx/nebulark:Z -e WORKDIR=/root/go/src/github.com/pojntx/nebulark/pkg/examples/cpp/json_calculator/ -e OUTDIR=/root/go/src/github.com/pojntfx/nebulark/web/examples/cpp/json_calculator/ pojntfx/wasi-sdk sh -c 'mkdir -p $$OUTDIR && cd $$WORKDIR && rm -rf build && mkdir -p build && cd build && cmake .. && make && cp calculator.wasm $$OUTDIR/main.wasm'
